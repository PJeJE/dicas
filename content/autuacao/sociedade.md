...
{{% notice info %}}
Por vezes, o usuário cadastrador tem em mãos um CNPJ e uma OAB da sociedade, mas a OAB da sociedade não está associada ao CNPJ no Cadastro Nacional de Sociedade de Advogados. Nessas situações, se pesquisar pela OAB, o sistema apresentará a mensagem **Erro ao tentar cadastrar o usuário: A sociedade pesquisada não tem CNPJ cadastrado na OAB**. Já se o usuário pesquisar pelo CNPJ, o sistema apresentará a mensagem **Não foi possível encontrar a sociedade a partir do parâmetro de pesquisa informado. Tente novamente.**. De fato, pelo CNPJ, não se consegue encontrar a sociedade, já que o CNPJ não está no Cadastro Nacional de Sociedade de Advogados. Não será possível cadastrar essa sociedade no sistema, já que CNPJ é uma informação obrigatória para o cadastro no PJe.
{{% /notice %}}
