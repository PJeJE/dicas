...
{{% notice note %}}
Para utilizar a variável de URL (`#{certidaoPublicacaoDJEService.getUrlVisualizarDocumento()}`), que permite acessar o documento diretamente por meio de um clique, consulte o vídeo explicativo disponível na documentação da [funcionalidade de certidão de publicação no Mural]({{< ref "certidao_mural.md" >}}#video-certidao-mural).
{{% /notice %}}
