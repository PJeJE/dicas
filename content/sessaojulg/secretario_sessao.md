...
A informação de órgão julgador acompanhado é onde o PJe registra se um órgão julgador acompanha outro órgão julgador no seu voto. Para o relator, essa informação armazena o mesmo órgão julgador relator do processo. 

Uma das situações de inconsistência apontadas nas [instruções atuais](/sessaojulg/secretario_sessao/#redistribuição-de-processo-quando-o-processo-já-está-em-tarefas-de-construção-de-documentos-de-sessão) é quando um processo muda de órgão julgador relator quando os documentos de sessão já estão em construção. Essa situação deve ser evitada ao máximo. Quando ocorrem esses casos, via de regra, o usuário percebe e ajusta o órgão julgador do voto do relator por meio da tarefa [Selecionar documentos para acórdão](/sessaljulg/selecionar_documentos). Caso o processo não tenha sido julgado ainda, mesmo com o ajuste mencionado realizado, ainda assim ele apresentará inconsistências em sessões de julgamento em que for pautado com aquele voto, já que o órgão julgador acompanhado continuará apontando para o relator anterior. A inconsistência fica explícita na exibição dos placares da sessão, já que o placar é o principal componente do sistema que leva a informação do órgão julgador acompanhado em consideração. A informação de órgão julgador acompanhado nunca é exibida nas telas do PJe, por mais que seja relevante para o correto funcionamento dos placares. Se houver necessidade, existe uma opção disponível na aba **Recursos e sessões** dos autos digitais que exibe a informação de órgão julgador acompanhado de cada voto do processo em uma sessão. A tela ajuda a elucidar casos que estejam gerando inconsistência nos placares e em tarefas de construção de voto. A exibição dos votos está condicionada ao papel **pje:sessao:permiteVisualizarVotos**.
...
{{% notice note %}}
Caso seja utilizada a tarefa [Selecionar documentos para acórdão](/sessaljulg/selecionar_documentos) para marcação de votos a serem utilizados na construção do acórdão, o usuário deve ter o cuidado de não selecionar o mesmo documento documento de voto em mais de uma aba da tarefa. Caso selecione, por exemplo, na aba **Voto relator** e na aba **Voto vencedor** o mesmo documento e o julgamento do processo tiver registrado um vencedor diferente do relator, o sistema permitirá, em tarefas de alteração do voto, que o usuário grave conteúdos por cima do mesmo documento pensando que está editando um documento novo. Conforme explicado na seção que detalha [Selecionar documentos para acórdão](/sessaljulg/selecionar_documentos), sua utilização deve ser sempre muito criteriosa porque pode causar diversas situações de inconsistência.
{{% /notice %}}
...
+ pauta onde o relator foi vencedor e órgão julgador do voto vencedor diferente do relator
+ mesmo documento vinculado a mesmo de um registro de documento de sessão. O documento do tipo Voto Relator de id 123 só pode estar associado a um órgão julgador e sessão. Esse mesmo documento não deve estar em outro registro de sessão com órgão julgador
